...
function statusWithFetch(
  ...
  if (providerID !== "openai" && !providerID.startsWith("openai-compatible") && !providerID.startsWith("opencode"))
    return { type: "unsupported", reason: "provider is not openai, opencode, or openai-compatible" }
  const npm = input.model.api.npm
  if (npm !== "@ai-sdk/openai" && npm !== "@ai-sdk/openai-compatible")
    return { type: "unsupported", reason: "provider package is not OpenAI or OpenAI-compatible" }
