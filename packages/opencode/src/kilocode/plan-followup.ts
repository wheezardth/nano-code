...
export const PlanFollowupRuntime = {
  ...
  async model(providerID: ProviderID, modelID: ModelID) {
  ...
  async loop(sessionID: SessionID) {
    return AppRuntime.runPromise(SessionPrompt.Service.use(svc => svc.loop({ sessionID })))
  },
}
...
export namespace PlanFollowup {
  ...
  async function resolveCodeModel(input: Pick<MessageV2.User, "model">) {
    if (Flag.KILO_CLIENT === "cli") {
      try {
        const raw = await Bun.file(path.join(Global.Path.state, "model.json")).text()
        const parsed = ModelState.safeParse(JSON.parse(raw))
        if (parsed.success) {
          const saved = parsed.data.model?.code
          if (saved) {
            ...
            if (full) {
              ...
              return { model: { ...saved, variant: resolveVariant(parsed.data.variant?.[key], full) } }
            }
          }
        }
      } catch { /* no saved model state */ }
    }
    ...
    if (entry?.model) {
      ...
      if (full) {
        return { model: { ...entry.model, variant: resolveVariant(entry.variant, full) } }
      }
    }
    return input
  }
    ...
    const listener = () =>
      input.question
        ...
        .then((qs) => {
          ...
          return input.question.reject(match.id)
        })
        .catch((error) => {
